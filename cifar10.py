...
tf.app.flags.DEFINE_string('data_dir', '/home/david/datasets/cifar10',
                           'where to store the dataset')

tf.app.flags.DEFINE_integer('dataset_seed', 1,
                           'dataset seed number')
...
def prepare_dataset():
    ...
    examples_per_class = int(NUM_LABELED_EXAMPLES / 10)
    labeled_train_images = np.zeros((NUM_LABELED_EXAMPLES, 3072), dtype=np.float32)
    labeled_train_labels = np.zeros((NUM_LABELED_EXAMPLES), dtype=np.int64)
    for i in xrange(10):
    ...
    rand_ix_labeled = rng.permutation(NUM_LABELED_EXAMPLES)
    ...
    train_images_valid, train_labels_valid = \
        labeled_train_images[NUM_VALID_EXAMPLES:], labeled_train_labels[NUM_VALID_EXAMPLES:]
...
def inputs(batch_size=100,
    ...
    filename_queue = generate_filename_queue(filenames, "/home/david/datasets/cifar10", num_epochs)
...
def unlabeled_inputs(batch_size=100,
    ...
    if validation:
        ...
        num_examples = NUM_EXAMPLES_TRAIN - NUM_VALID_EXAMPLES
    else:
